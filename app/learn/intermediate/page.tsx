...
export default async function IntermediatePage() {
  ...
  const courseModule = await getModuleLessons('intermediate', user.id);
  ...
  return (
    <div className="max-w-6xl mx-auto px-5 py-12">
      ...
      <div className="text-cyan-400 text-xs font-mono font-bold tracking-wider uppercase">GITNOVI / INTERMEDIATE</div>
      <h1 className="text-3xl sm:text-4xl font-extrabold mt-2 text-white">
        {courseModule?.title ?? 'Intermediate Git Collaboration'}
      </h1>
      <p className="text-slate-400 mt-2 text-sm sm:text-base max-w-2xl">
        {courseModule?.description ?? 'Collaboration, remote syncing, merging, conflict resolution, rebasing, and emergency recovery.'}
      </p>
